function [outputfiles] = cvr_cw_glm(image, mask, pco2)
        ...
        for j=1:image_size(2)
            for k=1:image_size(3)
                if(mask(i,j,k) == 1)
                    ...
                    ts(isnan(ts)) = 0;                      
                    f_ts = fft(ts');
                    cw = mscohere(real(ifft(f_pco2_hrf)),ts',[],[],Nr); 
                    p = polyfit(abs(f_pco2_hrf(1:floor(Nr/2+1))),cw'.*abs(f_ts(1:floor(Nr/2+1))),1);
                    cvr(i,j,k) = p(1);
                end
            end
        end
