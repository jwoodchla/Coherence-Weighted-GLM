function [outputfiles] = cvr_cw_glm(image, mask, pco2, TR)
    ...
    im_ts = mean(image, 1); im_ts = mean(im_ts, 2); im_ts = mean(im_ts, 3); im_ts = reshape(im_ts, [image_size(4) 1]);
    im_ts = im_ts./mean(im_ts)-1;
    
    % coherence weight calculation
    cw = mscohere(im_ts, pco2); 
    cw = imresize(cw, [center_index-1, 1]);
    cw = cw./sum(cw);
        ...
        for j=1:image_size(2)
            for k=1:image_size(3)
                if(mask(i,j,k) == 1)
                    ...
                    ts(isnan(ts)) = 0;                    
                    mag_ts = abs(fft(ts));  
                    b = polyfit(mag_co2(2:center_index).*scale_factor'.*cw ,mag_ts(2:center_index).*cw,1);
                    cvr(i,j,k) = b(1);
                end
            end
        end
